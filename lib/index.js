...
function collect (files, file, rebundle) {
  ...
  postcssify.collectTimer[file] = setTimeout(() => {
    if (files) {
      ...
      for (let i = 0; i < l; i++) {
        const index = i
        resolve(files[i], { filename: file, extensions: extensions }, (err, result) => {
          if (err) { throw err }
          files[index] = result
          if (!--cnt) {
            for (let i = 0; i < l; i++) {
...
function bundle () {
  ...
  if (ast) {
    const processor = postcss(nano ? processors.concat(cssnano(nanoOptions)) : processors)
    processor.process(ast.toResult(), { to: dest, map: !nano }).then((result) => {

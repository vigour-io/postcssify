...
function bundle () {
  ...
  // console.log('-- DEPS:', JSON.stringify(tr.deps, false, 2))
  var sorted = order(tr.deps[tr.entry], [])
  console.log('-- SORTED:', JSON.stringify(sorted, false, 2))
  // CONCAT: loop through sorted list => concat each body stored in cache
  var l = sorted.length
  var i = 0
  var ast

  // everything with string is TEMP
  var str = ''
  for (; i < l; i++) {
    let file = sorted[i]
    let node = parse(file)
    if (node) {
      ...
      if (~file.indexOf('/Users/youzi/dev/play-logo/node_modules/@vigour-io/css/lib/index.css')) {
        console.log('HA!', node.source.input.css) 
      }
      str += node.source.input.css
    }
  }
  if (ast || str) {
    var dest = 'bundle.css'

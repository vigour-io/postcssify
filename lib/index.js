...
function csstransform (file) {
  ...
  return through(function (buf, enc, next) {
    ...
    if (secondpass) {
      ...
      reader.on('close', () => {
        // let from = realpath(file)
        // styles[from] = postcss.parse(cssbody, { from: from }).toResult().css
...
function bundle () {
  ...
  processor.process(concat, { to: dest, map:true }).then((result) => {
    console.log('postcss:success!')
    fs.writeFile(dest, result.css, (err) => {
      if (err) { throw err }
      console.log('done writing to: ', dest)
    })
  }).catch((reason) => console.log('postcss:error!'))
}

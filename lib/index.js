...
module.exports = exports = function postcssify (brow, options) {
  ...
  options.filePlugins = typeof options.filePlugins === 'undefined'
    ? []
    : _isArray(options.filePlugins)
      ? options.filePlugins
      : [options.filePlugins]

  options.filePlugins = options.filePlugins.map(function (item) {
    return require(item)
  })

  fileProcessor = postcss(options.filePlugins)
  ...
  wrap.push(through(function (buf, enc, next) {
    ...
    exports.mergeASTs(options)
      ...
      .then((ast) => {
        ...
        var arr = [
          fs.writeFileAsync(options.out, ast.css, 'utf8')
        ]
        if (typeof options.map === 'string') {
          arr.push(fs.writeFileAsync(options.map, ast.map, 'utf8'))
        }
...
exports.sortRows = function (row, options, sorting, parent) {
  if (!sorting) {
    sorting = {}
  }
  if (parent) {
  ...
  if (!sorting[row.index]) {
    ...
    if (!_isEmpty(row.indexDeps)) {
      for (let key in row.indexDeps) {
        var pattern = 'require\\((?:(?:\'' + key + '\')|(?:"' + key + '"))\\)' // match the last occurrence of this require
      ...
      return Promise.all(deps.map((item) => {
        return exports.sortRows(bundle.meta[item.index], options, sorting, row.index)
      }))
        .then((vals) => {
...
exports.mergeASTs = function (options) {
  ...
  return Promise.resolve(exports.sortRows(entry, options))
    .then((vals) => {
      var unique = _reverse(_uniq(_reverse(vals))) // remove duplicates, keeping item nearest to the end
      return unique.reduce(function (prev, curr) {
        var ast
        return prev.then((_ast) => {
          ast = _ast
          var meta = bundle.meta[curr]
          var file = meta.file
          if (!bundle.files[file]) {
            if (bundle.meta[curr].sourceFile && bundle.meta[curr].sourceFile.slice(0, 3) === '../') {
              let parent = bundle.meta[meta.parent]
              let str = _findKey(parent.indexDeps, function findKeyCriterion (value) {
                return value === meta.index
              })
              file = path.join(parent.file.replace(parent.sourceFile, ''), 'node_modules', str)

              let lastPart = path.basename(file)
              if (lastPart.indexOf('.') === -1) {
                let searchTerm = '/' + lastPart + '/'
                let resolved = require.resolve(file)
                let idx = resolved.indexOf(searchTerm)
                if (idx !== -1) {
                  file = path.join(file, resolved.slice(idx + searchTerm.length))
                }
              }
            } else {
              throw new Error("Can't find file contents for " + file)
            }
          }
          return fileProcessor.process(bundle.files[file].buf.toString(), {
            from: file,
            to: options.out,
            map: true
          })
